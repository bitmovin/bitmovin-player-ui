...
function replaceAll() {
  var replacementStreams = replacements.map(function (replacement) {
    return replace(replacement[0], replacement[1]);
  });
...
gulp.task('html', function () {
  return gulp.src(paths.source.html).pipe(gulp.dest(paths.target.html));
});
...
gulp.task('sass', function () {
  var stream = gulp
    ...
    .pipe(postcss([autoprefixer(), postcssSVG()]))
    .pipe(cssBase64())
    .pipe(
...
gulp.task(
  'npm-prepare',
  gulp.series('build-prod', 'copy-json', function () {
