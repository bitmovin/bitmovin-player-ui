...
export class AdSkipButton extends Button<AdSkipButtonConfig> {
  ...
  configure(player: bitmovin.PlayerAPI, uimanager: UIInstanceManager): void {
    ...
    let adStartHandler = (event: bitmovin.PlayerAPI.AdStartedEvent) => {
      ...
      player.on(player.EVENT.ON_TIME_CHANGED, updateSkipMessageHandler);
      player.on(player.EVENT.ON_CAST_TIME_UPDATED, updateSkipMessageHandler);
    };

    let adEndHandler = () => {
      player.off(player.EVENT.ON_TIME_CHANGED, updateSkipMessageHandler);
      player.off(player.EVENT.ON_CAST_TIME_UPDATED, updateSkipMessageHandler);
    };
