...
export class SeekBar extends Component<SeekBarConfig> {
  ...
  configure(player: bitmovin.PlayerAPI, uimanager: UIInstanceManager, configureSeek: boolean = true): void {
    ...
    player.on(player.Event.Ready, playbackPositionHandler);
    // update playback position when it changes
    player.on(player.Event.TimeChanged, playbackPositionHandler);
    // update bufferlevel when buffering is complete
    player.on(player.Event.StallEnded, playbackPositionHandler);
    // update playback position when a seek has finished
    player.on(player.Event.Seeked, playbackPositionHandler);
    // update playback position when a timeshift has finished
    player.on(player.Event.TimeShifted, playbackPositionHandler);
    // update bufferlevel when a segment has been downloaded
    player.on(player.Event.SegmentRequestFinished, playbackPositionHandler);

    this.configureLivePausedTimeshiftUpdater(player, uimanager, playbackPositionHandler);

    // Seek handling
    player.on(player.Event.Seek, () => {
      this.setSeeking(true);
    });
    player.on(player.Event.Seeked, () => {
      this.setSeeking(false);
    });
    player.on(player.Event.TimeShift, () => {
      this.setSeeking(true);
    });
    player.on(player.Event.TimeShifted, () => {
      this.setSeeking(false);
    });
    ...
    player.on(player.Event.PlayerResize, () => {
      this.refreshPlaybackPosition();
    });
  ...
  private configureLivePausedTimeshiftUpdater(player: bitmovin.PlayerAPI, uimanager: UIInstanceManager,
    ...
    player.on(player.Event.Play, () => pausedTimeshiftUpdater.clear());
  }
  ...
  private configureSmoothPlaybackPositionUpdater(player: bitmovin.PlayerAPI, uimanager: UIInstanceManager): void {
    ...
    player.on(player.Event.Play, startSmoothPlaybackPositionUpdater);
    player.on(player.Event.Playing, startSmoothPlaybackPositionUpdater);
    player.on(player.Event.Paused, stopSmoothPlaybackPositionUpdater);
    player.on(player.Event.PlaybackFinished, stopSmoothPlaybackPositionUpdater);
    player.on(player.Event.Seeked, () => {
      currentTimeSeekBar = player.getCurrentTime();
    });
  ...
  private configureMarkers(player: bitmovin.PlayerAPI, uimanager: UIInstanceManager): void {
    ...
    player.on(player.Event.Ready, setupMarkers);
    // Remove markers when unloaded
    player.on(player.Event.SourceUnloaded, clearMarkers);
    // Update markers when the size of the seekbar changes
    player.on(player.Event.PlayerResize, () => this.updateMarkers());
