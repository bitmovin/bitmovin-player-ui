...
/**
 * Configuration interface for a {@link ModernSettingsPanel}.
 */
export interface ModernSettingsPanelConfig extends ContainerConfig {
  /**
...
/**
 * A panel containing a list of {@link ModernSettingsPanelPage items}.
 *
 * To configure pages just pass them in the components array.
 *
 * Example:
 *  let modernSettingsPanel = new ModernSettingsPanel({
 *    hidden: true,
 *  });
 *
 *  let modernSettingsPanelPage = new ModernSettingsPanelPage({
 *    components: […]
 *  });
 *
 *  let secondSettingsPanelPage = new ModernSettingsPanelPage({
 *    components: […]
 *  });
 *
 *  modernSettingsPanel.addComponent(modernSettingsPanelPage);
 *  modernSettingsPanel.addComponent(secondSettingsPanelPage);
 *
 * For an example how to navigate between pages @see SettingsPanelPageNavigatorButton
 */
export class ModernSettingsPanel extends Container<ModernSettingsPanelConfig> {

  private static readonly CLASS_ACTIVE_PAGE = 'active';

  // navigation handling
  private activePage: ModernSettingsPanelPage;
  private navigationStack: ModernSettingsPanelPage[] = [];

  private settingsPanelEvents = {
    onSettingsStateChanged: new EventDispatcher<ModernSettingsPanel, NoArgs>(),
  };

  private hideTimeout: Timeout;

  constructor(config: ModernSettingsPanelConfig) {
    super(config);
   ...
   * @return {ModernSettingsPanelPage}
   */
  getActivePage(): ModernSettingsPanelPage {
   ...
   * @params page
   */
  setActivePage(targetPage: ModernSettingsPanelPage): void {
    ...
    this.navigateToPage(
      ...
      !(this.config as ModernSettingsPanelConfig).pageTransitionAnimation,
    );
  }
  ...
  popToRootSettingsPanelPage(): void {
    this.resetNavigation((this.config as ModernSettingsPanelConfig).pageTransitionAnimation);
  }
   ...
   * @returns {ModernSettingsPanelPage[]}
   */
  getPages(): ModernSettingsPanelPage[] {
    return <ModernSettingsPanelPage[]>this.config.components.filter(component => component instanceof ModernSettingsPanelPage);
  }

  get onSettingsStateChanged(): Event<ModernSettingsPanel, NoArgs> {
  ...
  addComponent(component: Component<ComponentConfig>) {
    if (this.getPages().length === 0 && component instanceof ModernSettingsPanelPage) {
  ...
  private updateActivePageClass(): void {
    this.getPages().forEach((page: ModernSettingsPanelPage) => {
      if (page === this.activePage) {
        page.getDomElement().addClass(this.prefixCss(ModernSettingsPanel.CLASS_ACTIVE_PAGE));
      } else {
        page.getDomElement().removeClass(this.prefixCss(ModernSettingsPanel.CLASS_ACTIVE_PAGE));
      }
    });
  }
   ...
   * the navigation within the onShow callback of the modernSettingsPanel. In this case we don't want an actual animation but
   * the recalculation of the dimension of the modernSettingsPanel.
   * This is independent of the pageTransitionAnimation flag.
   */
  private animateNavigation(targetPage: ModernSettingsPanelPage, sourcePage: ModernSettingsPanelPage, skipAnimation: boolean) {
    if (!(this.config as ModernSettingsPanelConfig).pageTransitionAnimation) {
      return;
    }
  ...
  private hideHoveredSelectBoxes(): void {
    this.getComputedItems().forEach((item: ModernSettingsPanelItem) => {
  ...
  private getComputedItems(): ModernSettingsPanelItem[] {
    const allItems: ModernSettingsPanelItem[] = [];
  ...
  private getRootPage(): ModernSettingsPanelPage {
