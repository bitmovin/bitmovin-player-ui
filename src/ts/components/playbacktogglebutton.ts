  ...
  configure(player: bitmovin.player.Player, uimanager: UIManager, handleClickEvent: boolean = true): void {
    ...
    let playbackStateHandler = function(event: PlayerEvent) {
      ...
      if (player.isCasting() && event &&
        ...
        || event.type === bitmovin.player.EVENT.ON_CAST_PLAYING || event.type
        === bitmovin.player.EVENT.ON_CAST_PAUSED)) {
        isPlaying = !isPlaying;
      }
    ...
    player.addEventHandler(bitmovin.player.EVENT.ON_PLAYBACK_FINISHED, playbackStateHandler); // when playback
                                                                                              // finishes, player turns
                                                                                              // to paused mode
