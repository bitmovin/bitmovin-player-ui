...
declare namespace bitmovin {
    ...
    namespace player {
        ...
        interface Player {
             ...
             * @param subtitleTrackID A unique ID for the subtitle track. If the ID already exists, the existing track is overwritten with the new one.
            ...
            castVideo(): Player;
            /**
             * Removes all existing query parameters as specified in {@link #setQueryParameters} or {@link Config#tweaks#query_parameters}.
             */
             ...
             *
             * @param mergedConfig true to return the config expanded with all default values, false to return the user config passed to {@link #setup}
             */
             ...
             * TODO convert adType to enum? (if ad systems should be pluggable and they are therefore not statically defined, keep it as string)
             ...
             * @param keepPersistent Flag to set the poster image persistent so it is also displayed during playback (useful for audio-only playback)
             */
